...
> **Trạng thái (2026-08-05)**: Phase 0-5 đã lập trình, `clasp push` lên Apps Script, và đã live-test thật trên Apps Script runtime qua Execution API (`clasp run` với OAuth Client riêng — không chỉ kiểm tra cú pháp + tham chiếu chéo tĩnh nữa). Dashboard đã có giá trị nghiệp vụ riêng (Approval Queue + Document Log dùng dữ liệu thật, không còn placeholder). Đã phát hiện và fix qua live-test: RULES sheet đụng cột giữa 2 loại rule, lỗi export Word qua `getAs()`, dữ liệu Rule phân loại thiếu ở hệ thống khởi tạo trước khi có tính năng phân loại, quyền `script.send_mail` chưa được cấp lại sau khi thêm scope mới. Đã bổ sung ngoài phạm vi Phase 5 ban đầu: thông báo email cho luồng duyệt/từ chối (Workflow + Knowledge Governance), báo cáo độ chính xác phân loại AI (dùng dữ liệu `ClassificationFeedback` đã thu thập sẵn), xoá mềm + khôi phục tài liệu (`Status=ARCHIVED`, có màn hình quản lý riêng cho Admin), ghim phiên bản CDN (Alpine.js kèm SRI, Tailwind) thay vì dùng bản mới nhất thả nổi. Phase 6 (go-live, đào tạo người dùng) chưa bắt đầu, đúng thực tế vì cần vận hành thật mới thực hiện được. Chi tiết implementation bám sát thiết kế nhưng có vài đơn giản hoá thực dụng so với thiết kế ban đầu (ví dụ Workflow chạy 1 bước duyệt cố định thay vì diễn giải JSON động — xem comment đầu `src/08_Workflow/Workflow.Engine.gs`; Rule Engine chưa kiểm tra bố cục 2 cột trong `NATIONAL_HEADER` — xem docs/08-rule-engine.md mục 7, quyết định giữ nguyên theo YAGNI).
