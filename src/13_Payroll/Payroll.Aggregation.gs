...
function exportPayrollAggregationToExcel(actingUser, departmentId, yearMonth) {
  ...
  const headers = ['Mã NV', 'Họ tên', 'Ngày công', 'Ngày nghỉ', 'Số ca trực', 'Giờ làm thêm', 'Giờ làm ngoài giờ'];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  if (rows.length > 0) {
    const values = rows.map(function (r) {
      return [r.EmployeeCode, r.FullName, r.WorkDays, r.LeaveDays, r.DutyShiftCount, r.OvertimeHours, r.OutOfHoursHours];
    });
